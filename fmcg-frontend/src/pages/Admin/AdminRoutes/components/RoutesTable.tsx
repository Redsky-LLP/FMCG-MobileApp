...
// UPDATED:
// 1. Removed Override button — Assign is the better UX (permanent assignment with date picker covers both cases)
// 2. Added duplicate route name warning: if salesman already has a route with same name, shows warning before assign
...
export function RoutesTable({
  ...
  // Detect duplicate route names for same salesman
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {routes.map(r => {
          ...
          return (
            <div key={r.id} style={{
              background: '#fff', border: dupWarning ? '1px solid #FDE68A' : '1px solid #E2E8F0',
              ...
              {dupWarning && (
                <div style={{
                  ...
                  <span style={{ fontSize: 11, color: '#92400E', lineHeight: 1.4 }}>{dupWarning}</span>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12, fontSize: 12 }}>
                ...
                {r.createdAt && <span style={{ color: '#94A3B8' }}>Created {fmtDate(r.createdAt)}</span>}
              </div>

              {/* Row 3: actions — Override removed */}
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
      <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <table style={{ width: '100%', minWidth: 640, borderCollapse: 'collapse', fontSize: 14, background: '#fff' }}>
          <thead>
            <tr>
          ...
          <tbody>
            {routes.map(r => {
              ...
              return (
                <tr key={r.id}
                  style={{ borderBottom: '1px solid #F1F5F9', transition: 'background 0.12s', background: dupWarning ? '#FFFBEB' : 'transparent' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = dupWarning ? '#FEF9C3' : '#FAFBFD'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = dupWarning ? '#FFFBEB' : 'transparent'}
                >
                  {/* Route name */}
                      ...
                      <div>
                        ...
                        {dupWarning && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                            ...
                            <span style={{ fontSize: 11, color: '#92400E' }}>{dupWarning}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Actions — Override removed, only Assign + Edit + Delete */}
